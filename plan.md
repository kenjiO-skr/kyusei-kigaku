...
- [x] ブラウザ目視（追従・クランプ・復帰・保存・相性）＋ユーザー確認 OK → コミット・push（6c6dd20）

## Phase 12 SW更新の自動反映（2026-06-12）
- [x] index.html の SW 登録に controllerchange リスナーを追加：新版 SW が制御を引き継いだら自動 location.reload()（「2回起動しないと反映されない」問題の解消）
- [x] ガード2種：初回訪問（hadController=false）では再読み込みしない／refreshed フラグで多重リロード防止
- [x] 検証：初回訪問でループなし・SW更新発火で自動再読み込み（ページ遷移なしの registration.update() で実証）・新キャッシュのみ残存
- [x] SW v7 バンプ。テスト96件全緑
